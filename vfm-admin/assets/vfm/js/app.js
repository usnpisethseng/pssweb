...
function loadImg(thislink, thislinkencoded, thisname, thisID, ext){
    ...
    if (modalzoomview && vfmmodals.hasOwnProperty('zoomview')) {
        ...
        $(".vfm-zoom").html('<div class="prezoomimg"><div class="position-absolute w-100 h-100 start-0 top-0 d-flex justify-content-center align-items-center"><div class="spinner-border" role="status"></div></div></div><img class="preimg" src="vfm-admin/include/vfm-thumb.php?thumb='+ thislink +'&y=1"/>');
...
// lengthMenu : [[5, 10, 25, 50, 100, 200], [5, 10, 25, 50, 100, 200]],
    ...
    */
    ...
    if (filetableconfig.paginate == 'on' && filetableconfig.top_pagination == 'on') {
        filesdom += '<"table-controls-bottom"ip>'; // top pagination
    }

    filesdom += 'rt';

    if (filetableconfig.paginate == 'on') {
        filesdom += '<"table-controls-bottom"ip>'; // bottom pagination
    }
    ...
    */
    ...
    if (foldertableconfig.paginate == 'on') {
        ...
        if (foldertableconfig.top_pagination == 'on') {
            folderdom += '<"table-controls-bottom"ip>'; // top pagination
        }
...
$(document).ready(function() {
    ...
    // // Switch dark mode
    // $('body').append('<div class="dark-toggler">Dark Mode</div>');

    // var predarkMode = 'dark-mode';
    // if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
    //     var predarkMode = 'light-mode';
    // }

    // var darkToggler = $('.dark-toggler');

    // darkToggler.on('click', function(){
    //     sessionStorage.setItem('key', 'value');
    //     $('body').toggleClass(predarkMode);
    //     if ((darkToggler).hasClass(predarkMode)){
