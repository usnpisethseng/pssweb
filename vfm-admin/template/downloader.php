        ...
        if ($downloader->checkTime($time) == true) {
            ...
            foreach ($pieces as $count => $pezzo) {
                ...
                if (file_exists($myfile)) {
                    ...
                    if ($typeclass === 'thumb') {
                        $thumb = '<a '.$imgdata.' class="btn btn-primary p-0 m-0 '.$typeclass.' vfm-gall service-btn d-flex align-items-center justify-content-center" href="'.$downlink.'"><img style="height:calc(3.5rem + 2px); width:calc(3.5rem + 2px); max-width:none;" src="'.$imageServer->showThumbnail(base64_decode($pezzo), true).'?in=1"></a>';
                    } else {
                        $thumb = '<a style="width:calc(3.5rem + 2px);" class="btn btn-primary service-btn d-flex align-items-center justify-content-center" href="'.$downlink.'" '.$target.'><i class="bi bi-'.$thisicon.'"></i></a>';
                    }
